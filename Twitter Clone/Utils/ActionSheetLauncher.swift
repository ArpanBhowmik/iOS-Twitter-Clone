...
class ActionSheetLauncher: NSObject {
    ...
    private let tableViewHeight = (2 * 60) + 20 //(no of row * row Height) + footer height + some extra padding
    ...
    func show() {
        UIView.animate(withDuration: 0.5) {
            ...
            self.tableView.frame.origin.y -= CGFloat(self.tableViewHeight)
        }
    }
    ...
    private func configureTableView() {
        ...
        tableView.frame = CGRect(x: 0, y: window.frame.height, width: window.frame.width, height: CGFloat(tableViewHeight))
